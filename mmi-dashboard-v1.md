...
surveilr shell sql/cms-sleep-apnea-market-analysis.sql 
spry sp spc --package --conf sqlpage/sqlpage.json -m mmi-dashboard-v1.md | sqlite3 resource-surveillance.sqlite.db
...
SELECT 'shell' AS component,
    ...
    '{"link":"' || CASE WHEN instr(sqlpage.path(), 'mmi/') > 0 THEN '../' ELSE './' END || '","title":"Home"}' AS menu_item,
       ...
       '{"link":"/mmi/sleep-apnea-cms-evidence.sql","title":"Sleep Apnea Market Evidence"}' AS menu_item,
...
SELECT 'card' AS component, 'Analytics Modules' AS title, 4 AS columns;
...
SELECT 
    'Sleep Apnea Market Evidence' AS title,
    'Strategic analysis of the market: Mapping diagnostic density, DME rental caps, and NP referral networks.' AS description,
    '/mmi/sleep-apnea-cms-evidence.sql' AS link,
    'microscope' AS icon,
    'blue' AS color;
...
## Sleep Apnea Market Evidence

```sql mmi/sleep-apnea-cms-evidence.sql { route: { caption: "Sleep Apnea Market Evidence" } }
-- @route.description "Clusters mapped for opportunity scoring, excluding General/Other categories."
...
-- 2. Data Source Header
SELECT 'hero' AS component,
       'Medicare Sleep Apnea Market Evidence' AS title,
       'This report synthesizes Part B and DME datasets to prove the economic necessity of an AI-native, uniform-priced diagnostic model.' AS description;

-- 3. Executive Summary (High-Impact Cards)
SELECT 'title' AS component, 'Market Strategy — Key Evidence Metrics' AS contents;
SELECT 'card' AS component, 4 AS columns;

-- Use the view to populate cards dynamically
SELECT Metric AS title, CMS_Value AS footer, Voxia_Implication AS description, 
       CASE WHEN ID IN (1,6) THEN 'blue' WHEN ID=2 THEN 'red' ELSE 'indigo' END AS color
FROM complete_evidence_summary;

-- 4. Market Composition Chart (FIXED PIE RENDERING)
SELECT 'chart' AS component, 'Market Share: Diagnostics vs Treatment' AS title, 'pie' AS type;
SELECT 
    Dataset AS label, 
    -- Strip '$' and 'M' and cast to float for the chart engine
    CAST(REPLACE(REPLACE(REPLACE("Medicare Allowed", '$', ''), 'M', ''), ',', '') AS FLOAT) AS value 
FROM market_analysis_summary 
WHERE Dataset != 'COMBINED TOTAL';

-- 5. Market Analysis Summary Table
SELECT 'table' AS component, 'Market Economics Summary' AS title, 'Comparison of Part B vs DME segments' AS description;
SELECT * FROM market_analysis_summary;

-- 6. Interaction Density (The most important finding)
SELECT 'table' AS component, 
       'Interaction Density: One-and-Done vs. Chronic Rental' AS title,
       'High density (7.6+) identifies the rental model revenue ceiling.' AS description;
SELECT 
    Data_Code AS "Data / Code",
    PRINTF('%,.0f', Claims) AS "Claims",
    PRINTF('%,.0f', Beneficiaries) AS "Beneficiaries",
    ROUND(Density, 2) AS "Density",
    Revenue_Implication AS "Revenue Implication",
    CASE 
        WHEN Data_Code LIKE '%CPAP%' THEN 'table-warning'
        ELSE 'table-light'
    END AS _sqlpage_css_class
FROM dme_interaction_density;

-- 7. Referring Specialty Analysis
SELECT 'table' AS component, 'Referring Specialty — Partnership Map' AS title, 'Top referral sources' AS description;
SELECT 
    Specialty,
    Providers,
    PRINTF('%,.0f', Services) AS Services,
    PRINTF('%.1f%%', Pct_Allowed) AS "% Allowed",
    Voxia_Strategy AS "Voxia Strategy",
    CASE 
        WHEN Specialty IN ('Nurse Practitioner', 'Physician Assistant') THEN 'table-success'
        WHEN Specialty = 'Pulmonary Disease' THEN 'table-primary'
    END AS _sqlpage_css_class
FROM referring_specialty_analysis;

-- 8. Geographic Disparity
SELECT 'table' AS component, 'Geographic Access — Urban vs Rural Disparity' AS title;
SELECT 
    RUCA_Category AS "Category",
    Providers,
    PRINTF('%,.0f', Services) AS "Total Services",
    PRINTF('$%,.1fM', Total_Allowed / 1000000.0) AS "Total Allowed",
    PRINTF('$%.2f', ROUND(Avg_Allowed_Per_Service, 2)) AS "Avg Cost/Service",
    CASE WHEN RUCA_Category = 'Rural' THEN 'text-danger font-weight-bold' END AS _sqlpage_css_class
FROM geographic_access_analysis;

    ...
    messages?.forEach((m: any, i: number) => {
      if (Array.isArray(m.content)) {
        m.content.forEach((c: any, j: number) => {
          if (c.type === "image") {
            console.log(`Msg ${i} Part ${j}: Image type detected. Value starts with: ${typeof c.image === "string" ? c.image.substring(0, 50) : "not-a-string"}`);
          }
        });
      }
    });
...
- Writing JOINs without first confirming the join key columns exist in both tables.${tableHint}`;
   ...
   console.log("Converted message structure:", JSON.stringify(
  convertedMessages.map((m: any) => ({
    role: m.role,
    content: Array.isArray(m.content) 
      ? m.content.map((c: any) => ({ type: c.type, keys: Object.keys(c) }))
      : typeof m.content
  }))
, null, 2));
...
const sanitizedMessages = convertedMessages.map((m: any) => {
  ...
  return {
    ...
    content: m.content.map((c: any) => {
      // Check both possible locations of the image value
      ...
      if (
        ...
        try {
          ...
          console.log("Sanitizing image with mimeType:", mimeType);
          return {
            type: "image",
          ...
          console.error("Failed to sanitize image part:", e);
          return c;
        }
      }
      return c;
